...
const ModifierProfil = () => {
  ...
  const handleUpdateUser = async () => {
    try {
      ...
      if (values && values.phonenumber && selectedCountry) {
        ...
        const phoneNumberWithoutRepeatedCountryCode =
          formattedPhoneNumber.replace(
            new RegExp(`\\+${values.countryCode} `, "g"),
            "+"
          );
        ...
        // Construire les données utilisateur à mettre à jour
        ...
        // Envoyer la requête de mise à jour en fonction du rôle
        let response;
        if (role === "client") {
          response = await apiusers.updateClient(idProfil, userDataToUpdate);
          fetchUser(); // Mettre à jour les données utilisateur après mise à jour financière
        } else if (role === "financier") {
          response = await apiusers.updateFinancier(idProfil, userDataToUpdate);
          fetchUser(); // Mettre à jour les données utilisateur après mise à jour financière
        } else {
          console.error("Rôle utilisateur inconnu :", role);
          message.error("Rôle utilisateur inconnu. Veuillez réessayer.");
          return;
        }
        ...
        if (response) {
          ...
          form.resetFields(); // Réinitialiser le formulaire après la mise à jour réussie
          setModalVisible(false); // Cacher le modal après mise à jour réussie
        } else {
        ...
        console.error("Données manquantes ou invalides :", values);
        message.error("Données manquantes ou invalides.");
      }
    } catch (error) {
      console.error("Erreur lors de la mise à jour de l'utilisateur :", error);

      // Inspecter les détails de l'erreur de réponse
      if (error.response) {
