  ...
  useEffect(() => {
    const fetchClient = async () => {
      try {
        ...
        console.log("ID du profil récupéré :", idProfil);

        const response = await axios.get(
          `http://localhost:5000/api/clients/${idProfil}`
        );
        console.log("Réponse de l'API (client) :", response);

        const clientFacturesData = response.data.factures.map((facture) => {
          // Traitement des services de la facture
          const services = facture.services.map((service) => ({
          ...
          // Traitement des paiements associés s'ils existent
          const paiements = facture.paiemnts || [];
          const paiementsFormatted = paiements.map((paiement) => ({
            typepaiement: paiement.typepaiement || "Non spécifié",
            datepaiement: paiement.datepaiement || "En cours",
            etatpaiement: paiement.etatpaiement || "Non payé",
          ...
          return {
            ...
            devise: facture.devise ? facture.devise.symbole : "Non spécifié",
  ...
  const columns = [
    {
      ...
      dataIndex: "date",
      key: "date",
      width: 80, // Largeur fixe pour la date
      render: (date) => {
    ...
    },
    {
      ...
      key: "numeroFacture",
      width: 100, // Largeur fixe pour le numéro de facture
    },
    {
      title: "Total TTC",
      dataIndex: "totalTTC",
      key: "totalTTC",
      width: 100, // Largeur fixe pour le total TTC
    },
    {
      title: "Paiements",
      key: "paiements",
      width: 200, // Largeur fixe pour la colonne des paiements
      render: (_, record) => (
        <div style={{ maxHeight: "200px", overflowY: "auto" }}>
          {record.paiements.map((paiement, index) => (
            <div
              key={index}
              style={{
                marginBottom: "10px",
            ...
            >
              <p>
                <strong>Type de paiement:</strong> {paiement.typepaiement}
              </p>
              <p>
              ...
              <p>
                <strong>État de paiement:</strong> {paiement.etatpaiement}
              </p>
            </div>
          ))}
        </div>
    ...
    },
    {
      title: "Action",
      key: "action",
      width: 100, // Largeur fixe pour la colonne des actions
      render: (_, record) => (
        <Space>
          <button
            style={{
